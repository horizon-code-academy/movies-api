...
module.exports = (router) => {
    ...
    router.get("/films/:pagenum", (request, response) => {
        filmModel.paginate({}, { page: request.params.pagenum, limit: 3 })
            .then((result) => {
                response.send(result);
            })
            .error((error) => {
    ...
    router.put("/film/:id", async (request, response) => {
        try {
            filmModel.findOneAndUpdate({ _id: request.params.id }, request.body, (err, updatedFilm) => {
                response.send(updatedFilm);
            });
        } catch (error) {
            console.error(error)
            response.status(500).send(error)
